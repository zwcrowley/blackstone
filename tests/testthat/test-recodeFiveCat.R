test_that("function outputs expected data types and structure", {
    ...
    df_expected <- dplyr::tibble(Pre_Orgs = c(1,2,3,4,5,4,3,2,1), Post_Orgs = c(1,2,3,4,5,4,3,2,1),
    ...
    df_expected <- df_expected %>% dplyr::mutate(cat_Pre_Orgs = factor(.data$cat_Pre_Orgs, levels = levels_min_ext),
                                                 cat_Post_Orgs = factor(.data$cat_Post_Orgs, levels = levels_min_ext))

    expect_equal(cat_items_1, df_expected)
    expect_equal(str(cat_items_1), str(df_expected))
})
