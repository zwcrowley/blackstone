...
new <- clean_items_single_div %>%
    ...
    ggplot2::guides(custom = ggplot2::guide_custom(grob = legendGrobCustom(names(new_fill_colors_named), nrow = 1, # using scale_labels and one row
                                                                           pch = 22, # pch 22 is a filled square
                                                                           sym_size = grid::unit(0.75, "cm"), # new size par for sizing pch symbol 
...
ggplot(mtcars, aes(x = mpg, y = wt, shape = factor(gear), fill = factor(gear))) +
      ...
      scale_shape_manual(values = rep(22, length(unique(mtcars$gear)))) + 
