...
fill_colors  <- "div"
    ...
    # new_df %>% dplyr::mutate(., label_color = map_chr(.data[["response"]], \(x) label_colors_named[x])) %>% 
    ...
    diverging_bar_chart <- new_df %>%
        ...
        ggplot2::scale_fill_manual(values = new_fill_colors_named, labels = NULL) + # turn off labels in legend
        ggplot2::guides(fill = ggplot2::guide_legend(
            ...
            override.aes = list(color = label_colors_named, # manually sets the color of the legend text to white or black
                                label = names(new_fill_colors_named),
                                fill = new_fill_colors_named) # manually sets the legend labels to wrapped scale_labels
        )) +
    ...
    # }
    # 
    # # Final call to stackedBar_ggplot() for chart creation:
    #     diverging_bar_chart <- stackedBar_ggplot(df_gg = new_df, y_gg = "timing", pre_post = TRUE,
