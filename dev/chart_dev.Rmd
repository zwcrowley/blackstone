...
question_labels <- NULL
    ...
    split_level <-  ceiling(length(scale_labels)/2) # split the response var so that more levels on on the bottom
    top_num_level <- length(scale_labels) - split_level # get the top number of levels
    top_levels <- tail(scale_labels, top_num_level) # get the top scale levels
    bottom_levels <- rev(head(scale_labels, split_level)) # reversed the bottom levels
