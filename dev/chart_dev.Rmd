...
names(new_fill_colors) <- scale_labels

grid.newpage()
## These *do* nothing but produce a "gpar" object:
gp <- gpar(col = "black", fontsize = 10, fontfamily = "Arial")
vp <- viewport(width = .8, height = .4)
g <- gTree(children = gList(rectGrob(gp = gpar(col = "black", lwd = 1.2, fill = new_fill_colors[1]),x = 0, y = 1, width = unit(0.6, "cm"), height = unit(0.6, "cm")),
                            textGrob(paste("The rect is its own colour (red)",
                                    "but this text is the colour",
                                    "set by the gTree (green)",
                                    sep = "\n"))),
                            rectGrob(gp = gpar(col = new_fill_colors[2]),x = 0.1, y = 1, width = unit(0.6, "cm"), height = unit(0.6, "cm")),
      gp = gp, vp = vp)
grid.draw(g)
...
new <- clean_items_single_div %>%
    ...
    ggplot2::guides( #fill = NULL 
                    fill = ggplot2::guide_legend(nrow = 1),
                    custom = guide_custom(circle, title = "My circle",
