...
# 
# # Recode the numeric to factor variables using the levels from levels_min_ext and Select the factor variables:
# cat_items_single_three <- bre::recodeCat(items_single_three, levels_min_ext_named) %>% dplyr::select(dplyr::where(is.factor))
...
question_labels <- NULL
    ...
    stacked_bar_chart_gg <- {{ df_gg }} %>%
        ...
        ggplot2::scale_fill_manual(values = new_fill_colors_named_unwrapped, breaks = names(new_fill_colors_named_unwrapped), 
                                   limits = names(new_fill_colors_named_unwrapped), # names(new_fill_colors_named_unwrapped),
                                    drop = FALSE, labels = NULL) + 
        ggplot2::scale_color_identity(limits = names(label_colors_named), labels = label_colors_named, drop = FALSE)
    if (isTRUE(pre_post)) {
        ...
        stacked_bar_chart_gg <-  stacked_bar_chart_gg  + # turn off labels in legend
            ...
            #                         label = legend_labels,
            #                         fill = new_fill_colors_named_unwrapped) # manually sets the legend labels to wrapped scale_labels
            # )) +
