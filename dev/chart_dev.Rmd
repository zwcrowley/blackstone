...
cat_items_four <- blackstone::recodeCat(items_four , levels_min_good_named) %>% dplyr::select(dplyr::where(is.factor))
...
```{r}
items_single_three <- dplyr::tibble(
  Organization = c(3, 4, 3, 4, 5, 4, 3, 4, 3),
  Source = c(4, 4, 3, 5, 4, 3, 4, 3, 4),
  Publish = c(3, 3, 3, 4, 4, 4, 3, 3, 3),
  Write = c(4, 4, 4, 3, 3, 3, 4, 4, 4),
  Research = c(3, 3, 4, 4, 3, 3, 4, 4, 4),
)

# Recode the numeric to factor variables using the levels from levels_min_ext and Select the factor variables:
cat_items_single_three <- blackstone::recodeCat(items_single_three, levels_min_ext_named) %>% dplyr::select(dplyr::where(is.factor))
df <- cat_items_single_three
...
question_labels <- NULL
    ...
    if (isTRUE(percent_label)) {
        ...
        pre_post = FALSE
        label_gg = label_gg
        label_colors_named = label_colors_named
        legend_labels = names(fill_colors_legend)
        width_gg = width 
        fill_colors_gg = fill_colors_named
        ...
        stacked_bar_chart_gg <-  stacked_bar_chart_gg  + # turn off labels in legend
            ggplot2::guides(fill = ggplot2::guide_legend(
                ...
                override.aes = list(color = label_colors_named, # manually sets the color of the legend text to white or black
                                    ...
                                    fill = fill_colors_gg
                                    ) # manually sets the legend labels to wrapped scale_labels
            )) +
...
blackstone::stackedBarChart(
   df = cat_items_single_three, pre_post = F, scale_labels = levels_min_ext, overall_n = T,
   question_labels = NULL, question_order = F, percent_label = TRUE, width = NULL
)

blackstone::divBarChart(
   df = cat_items_single_three, pre_post = F, scale_labels = levels_min_ext, overall_n = T,
   question_labels = NULL, question_order = F, percent_label = TRUE, width = NULL
)
```
...
```{r}
# df <- data.frame(x = c("a", "b", "c", "d"), y = c(0, 4, 1, 2))
df <- data.frame(x = c("b", "c", "d"), y = c(4, 1, 2))
bars <- ggplot(df, aes(x, y)) + 
  geom_col(aes(x, y, fill = x), position = "fill", show.legend = TRUE) + 
  labs(x = NULL, y = NULL) +
  scale_fill_manual(
    values = c("a" = "sienna1", "b" = "sienna4", "c" = "hotpink1", "d" = "hotpink4"),
    limits = c("a", "b" , "c" , "d"), 
    drop = FALSE
    # guide = guide_legend(override.aes = list(color = "black", # manually sets the color of the legend text to white or black
    #                                          fill = c("sienna1", "sienna4",  "hotpink1", "hotpink4"),
    #                                          labels = c("a", "b" , "c" , "d")))
  )
bars
# get_guide_data(bars, "fill")
...
# Fixing arrow chart question ordering:
...
### Update `blackstone::stackedBarChart()`

```{r}
# Start by processing the data using `blackstone::stackedBarChart()`:
