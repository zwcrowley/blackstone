...
sm_data <- sm_data %>% dplyr::mutate(dplyr::across(tidyselect::contains("_knowledge"), ~ factor(., levels = levels_knowledge)), # match each name pattern to select to each factor level
                                     ...
                                     # individually convert all demographics to facter variables:
...
freq_tables <- freq_params %>% purrr::pmap(blackstone::likertTable) %>% purrr::set_names(., c("Knowledge Items", "Reserach Items", "Ability Items", "Ethics Items"))
freq_tables
...
sm_data %>% dplyr::select(gender, ethnicity, first_gen) %>% # select the vars
                 blackstone::groupedTable(question_labels = demos_labels) # pass the new labeles for the 'Question' column.
```
