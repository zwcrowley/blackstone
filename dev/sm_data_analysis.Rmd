...
`stackedBarChart()` takes in a [tibble][tibble::tibble-package] of factor/character variables to turn into a stacked bar chart. The other requirement is a character vector of 
...
Both in a [tibble][tibble::tibble-package] of numeric pre-post data as the main input, and also require a character vector of scale labels for the numeric scale 
...
# df <- sm_data %>%  dplyr::select(tidyselect::contains("research_") & tidyselect::contains("_num") & where(is.numeric))
