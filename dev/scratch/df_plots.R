...
horz_bars_df <- map(selected_horz_bars, \(x) data %>% dplyr::select(all_of(x)) %>% blackstone::dataSumm(.,sort_n = TRUE) %>%
                        blackstone::horzBarChart(., scale_colors = mark_colors_six, width = 0.4)) %>% setNames(., selected_horz_bars)
...
create_sb_plot <- function(title, variables_names, levels, percent_label, overall_n) {
    ...
    stacked_bar <- data %>% dplyr::select(all_of(variables_names)) %>%
        blackstone::stackedBarChart(., pre_post = FALSE, scale_labels = levels_names, percent_label = percent_label, overall_n = overall_n)
    return(stacked_bar)
}
...
stacked_bars_list <- pmap(sb_options_df, \(title, vars, levels, percent_label, overall_n ) data %>% dplyr::select(all_of(vars)) %>%
                          blackstone::stackedBarChart(., pre_post = FALSE, scale_labels = levels, percent_label = percent_label, overall_n = overall_n)) %>%
                        set_names(., title_sb)
...
create_sb_plot <- function(df, id) {
    ...
    stacked_bar <- df %>% dplyr::select(all_of(variables_names)) %>%
        blackstone::stackedBarChart(., pre_post = FALSE, scale_labels = get(paste0("levels_", id)),
...
#   stacked_bar <- df %>% dplyr::select(all_of(variables_names)) %>%
#     blackstone::stackedBarChart(., pre_post = FALSE, scale_labels = levels_names,
...
# Function to create a stacked bar chart using blackstone::stackedBarChart() inside pmap() and a tibble/df of the inputs that
...
# Set up character vector of text or other things like punctuation to remove from the text data when using blackstone::openendedCleanup():
...
oe_tbl_list <- map(selected_columns_oe, \(x) data %>% dplyr::select(all_of(x)) %>% blackstone::openendedCleanup(., x, remove_values) %>%
...
map(..(selected_columns_oe()), \(x) ..(data()) %>% blackstone::openendedCleanup(., all_of(x), remove_values) %>%
        openendedFlextable(., header_label = str_glue("These are the responses from the question {x}"))) %>%
            setNames(., ..(selected_columns_oe()))

# Fix OE cleanup function from blackstone package:
