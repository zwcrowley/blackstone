...
```{r}
bre::stackedBarChart(
   df = cat_items_single_div, fill_color = "div", pre_post = FALSE, scale_labels = levels_dis_agree,
...
# clean_items_single_div %>% filter(question %in% c("Organization","Publish","Research")) %>% 
    ggplot2::ggplot(data = clean_items_single_div, mapping = ggplot2::aes(y = rev(question), x = percent_answers, group = question)) +
