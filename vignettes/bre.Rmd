...
These packages use the system of "tidy data" to make data manipulation and analysis in `R` consistent and well organized, `bre` also strives to follow this philosophy. 
...
Our current survey provider is [SurveyMonkey](https://www.surveymonkey.com/?ut_source=homepage&ut_source3=header), `bre` contains several functions that makes the process of 
...
Here is how to import data from SurveyMonkey using example data provided with `bre`, this is a fake dataset of a pre (baseline) survey. 
...
pre_data_fp <- bre::bre_example("sm_data_pre.csv")
# 1. Create the codebook:
codebook_pre <- bre::createCodebook(pre_data_fp)
codebook_pre
```
...
pre_data <- bre::readRenameData(pre_data_fp, codebook = codebook_pre)
pre_data
```
...
The vignette `Importing Data using bre` goes into deeper detail into using `bre` to read in data into `R`.
