...
[^demo]: The `blackstone` package contains a folder structure to emulate a standard Blackstone project folder, I had to change the naming conventions slightly to include it so the folder names are a bit different.
...
Now that we have an **RStudio Project** we can use it to save all of the files necessary for a data analysis task inside its folder.

## Managing File Paths with the `here` Package

Lets say we were tasked with the data analysis of a hypothetical Blackstone project for its fourth year (`Year 4 (2023-2024)`). 

We have completed the above steps to create an **RStudio Project** in the `Phase 2 - Evaluation/Data Analysis/Year 4 (2023-2024)` folder which we named 'analysis' as shown in the directory tree below, there is a subfolder of `Phase 2 - Evaluation/Data Analysis/Year 4 (2023-2024)` named analysis with an **RStudio Project** file named `analysis.Rproj`. 
...
To confirm which **`RStudio` Project** is open in your current `RStudio` instance go to the top-left corner of `RStudio`.

```{r current_proj, fig.cap = "Figure 5: Current RStudio Project", echo = FALSE}
