...
#'
#' @examples
#' items <- dplyr::tibble(cat_Pre_Sources = c("Good","Moderate","Minimal","Slight","Slight","Moderate","Good"),
...
stackedBarChart <- function(df, set_5_levels) {
    ...
    new_df <-  {{df}} %>%
        ...
        tidyr::separate(question, into = c("timing", "question"), sep = "_") %>%
        ...
        dplyr::ungroup() %>%
        dplyr::mutate(percent_answers_label = scales::percent(percent_answers, accuracy = 1),
               timing = factor(timing, levels = c("Pre","Post")),
               response = factor(response, levels = set_5_levels),
               label_color = dplyr::if_else(response == levels(response)[2], "black", "white"))

    N_df <- new_df %>% dplyr::select(-c(percent_answers,percent_answers_label)) %>%
        tidyr::pivot_wider(names_from = c(question,timing), values_from = c(n_answers)) %>%
    ...
    stacked_bar_chart <- new_df %>%
        ...
        ggplot2::coord_flip() +
        ggh4x::facet_nested_wrap(dplyr::vars(question,timing), ncol = 1, scales = "free", strip.position = "left",
