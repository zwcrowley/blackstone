...
```{r}
scale_labels = c("Minimal", "Slight", "Moderate", "Good", "Extensive")

tbl_likert_items <- merged_data %>%
    dplyr::select(cat_pre_organization:cat_post_research) %>% tidyr::drop_na() %>%
    ...
    dplyr::summarize(n_answers = dplyr::n(), .groups = "rowwise") %>%
    dplyr::ungroup() %>%
    ...
    dplyr::arrange(question, timing) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(percent_answers_label = scales::percent(.data$percent_answers, accuracy = 1)) %>% 
    as_grouped_data(x = ., groups = c("question", "timing"), columns = NULL, expand_single = TRUE) %>% 
    select(-percent_answers) %>% 
    flextable::as_flextable(., col_keys = c("question", "timing", "response", "n_answers", "percent_answers_label")) %>%
    # flextable::set_header_labels(n_answers = "#",
    #                              percent_answers_label = "%")  %>%
    flextable::align(j = 4:5, align = "center", part = "all") %>%
    ...
    flextable::bg(bg = "#4B9FA6", part = "header") %>%
...
```{r freq table for indiviual likert scale items}
# Try new code for freq table of multiple items that share the same resonse/scale_labels:
scale_labels = c("Minimal", "Slight", "Moderate", "Good", "Extensive")

tbl_likert_items <- merged_data %>%
    select(cat_pre_organization:cat_post_research) %>%
    tidyr::drop_na() %>%
    tidyr::pivot_longer(tidyselect::everything(), names_to = "Variable", values_to = "response") %>%
    dplyr::group_by(.data$Variable, .data$response) %>%
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(percent_answers_label = scales::percent(.data$percent_answers, accuracy = 1),
                  percent_answers_label = paste0("(",percent_answers_label,")"),
                   response = factor(.data$response, scale_labels)) %>%
  pivot_wider(-percent_answers,
    names_from = response,
    names_glue = "{response}_{.value}",
    names_vary = "slowest",
    values_from = c(n_answers, percent_answers_label),
    names_sort = TRUE,
  )  %>% mutate(across(contains('n_answers'), replace_na, 0),
                across(contains('percent_answers_label'), replace_na, "-")) %>% 
    janitor::adorn_totals("col", name = "n") %>% 
    unite(., col = !!sym(scale_labels[1]), starts_with(scale_labels[1]), na.rm = FALSE, remove = TRUE, sep = " ") %>% 
    unite(., col = !!sym(scale_labels[2]), starts_with(scale_labels[2]), na.rm = FALSE, remove = TRUE, sep = " ") %>% 
    unite(., col = !!sym(scale_labels[3]), starts_with(scale_labels[3]), na.rm = FALSE, remove = TRUE, sep = " ") %>% 
    unite(., col = !!sym(scale_labels[4]), starts_with(scale_labels[4]), na.rm = FALSE, remove = TRUE, sep = " ") %>% 
    unite(., col = !!sym(scale_labels[5]), starts_with(scale_labels[5]), na.rm = FALSE, remove = TRUE, sep = " ") 

tbl_likert_items

# tbl_likert_items %>% select(-"Variable", -"n") %>% 
#   lapply(function(x){unite(tbl_likert_items, x, starts_with(matches(scale_labels)), 
#                             sep = " ", remove = TRUE)}) %>%
#   bind_cols() %>% mutate(site = as.character(df$site)) %>% select(site, starts_with('D'))

# Display the freq and %:
# Pre
# merged_data %>%
#   select(cat_pre_organization:cat_post_research) %>%
#   dataSumm() %>% tblSumm()
...
# Post
# merged_data %>%
#   select(cat_Post_Confidence1:cat_Post_Confidence4) %>%
#   tbl_likert()
